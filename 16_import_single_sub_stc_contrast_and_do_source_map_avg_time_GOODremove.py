...
"""
=============================================
Source plots of evoked contrast 
...
#####################################################
#contrast_kind = ['NoGo_GOu'] # CHANGE HERE
time_pos = 'T4' # 'T0' | 'T1' | 'T2' | 'T3' | 'T4' CHANGE HERE
####################################################### 
for ei, evnt in enumerate(event_type):
    ...
    for veri, version in enumerate(version_list):
        ...
        for contra, contrast in enumerate(contrast_kind): # three contrasts
            ...
            report = mne.Report()
            print ('single sub STC average across time for ' + contrast + ' at ' + evnt)
            ...
            for sub_num, subject in enumerate(config_for_gogait.subjects_list): 
                ...
                clim = dict(kind="value", lims=[vmin, vmid, vmax])
                        ...
                        # # stc_min = np.min(stc_mean_timepts.data)
                        # # stc_max = np.max(stc_mean_timepts.data)
                ...
                stc_fig1 =  stc_mean_timepts.plot(
                                    ...
                                    clim = clim #dict(kind="percent", lims = (0, 50, 95)), # in percentile
                                )  
            ...
            report.title = 'single sub stc evkContrast averaged maps for ' + contrast + '_at_'+ evnt +'_' + timeDur + '_' + version +'_' + orientation + '_' + method 
            extension = 'single_sub_stc_evkContrast_averaged_maps_for_' + contrast
