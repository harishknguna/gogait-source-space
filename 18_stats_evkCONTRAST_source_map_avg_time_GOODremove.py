...
for ei, evnt in enumerate(event_type):
    ...
    for veri, version in enumerate(version_list):
        ...
        ## array stored in this order: contrast_kind = ['NoGo_GOc', 'NoGo_GOu', 'GOu_GOc']  
        # contrast_kind_reorder = ['GOu_GOc', 'NoGo_GOu', 'NoGo_GOc']
        ...
        for ti, time_pos in enumerate(time_points_kind): 
            ...
            for ci2, contrast in enumerate(contrast_kind):
                ...
                # baseline time
                tmin_bsl = np.where(times == find_closest(times, -0.2))[0][0]
                tmax_bsl = np.where(times == find_closest(times, 0.0))[0][0]
               
                if contrast == 'GOu_GOc':
                    #before: subs in dim1, condi in dim2, vertices in dim3, time in dim4  
                    #after: subs in dim1, vertices in dim2, time in dim3             
                    X1 = np.mean(stc_data_all_sub_all_condi[:,0,:,tmin_ind:tmax_ind],axis =2)  # all subs
                    X2 = np.mean(stc_data_all_sub_all_condi[:,0,:,tmin_bsl:tmax_bsl],axis =2)  # all subs
                    ...
                    X2 = np.mean(stc_data_all_sub_all_condi[:,1,:,tmin_bsl:tmax_bsl],axis =2)  # all subs
                    stc_contrast_data =  np.mean(X1 - X2, axis = 0) # averaged across subs
                    stc_contrast_data_std =  np.std(X1 - X2, axis = 0) # std across subs
                ...
                ## lims: takes only positive values 
                ## pos_lims: takes positive and negative values (mirrored)  
                ## reason why contrast, tstats be positive only values? 
                
                for plotType in range(4):
                    if plotType == 0:
                        stc_type = stc_contrast.copy()
                        clim=dict(kind="percent", lims = (0, 50, 95)) # in percentile
                        # use deault colormap 
                    elif plotType == 1:
                        ...
                        # clim=dict(kind="percent", lims = (0, 50, 95)) # in percentile
                        clim=dict(kind="value", lims = (7.01, 8.08, 9.24)) # in percentile
                        colormap = 'Purples'
                    elif plotType == 3:
                        stc_type = stc_pval.copy() #stc_pval.copy()
                        clim=dict(kind="value", lims = (6.30, 7.30, 8.30)) 
                        colormap = 'YlGn'
                    ...
                    else: #plotType == 1 or plotType == 2 or 3: 
                        stc_fig1 =  stc_type.plot(
                                            colormap = colormap,
                                            views=["lat"],
                                            hemi= "split",
        ...
        report.title = 'Group (n = ' + str(n_subs) + ') avg evkContrast stats maps at '  + evnt +': '+ version +  '_' + orientation + '_'  + method + '_' + ampliNormalization
        extension = 'group_avg_evkContrast_stats_maps' 
